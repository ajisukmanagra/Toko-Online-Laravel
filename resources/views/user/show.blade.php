			...
			<div class="row">
				<div class="col-lg-6">
					<div class="product-pic-zoom">
						<img class="product-big-img" src="{{ asset( $data->gambar ) }}" alt="">
					</div>
				</div>
				<div class="col-lg-6 product-details">
					<h2 class="p-title">{{ $data->type }}</h2>
					<h3 class="p-price">Rp. {{ number_format($data->price, 0) }}</h3>
					<h2 class="p-title">katagori : <span>{{ $data->merek->name }}</span></h2>
					...
					<a href="{{ url('add-to-cart/'.$data->id) }}" class="site-btn">SHOP NOW</a>
				</div>
			</div>
		</div>
	</section>
